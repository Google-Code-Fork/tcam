...
bool CPacket::Allocate(unsigned int memCount)
{
...
void CPacket::Advance(unsigned int nSize)
{
	offset += nSize;
}

void CPacket::Write(char *cBuffer, int nSize)
{
	memcpy(&data[offset],cBuffer,nSize);
	offset += nSize;
}

void CPacket::Write(int iBuffer, int nSize)
{
	memcpy(&data[offset],&iBuffer,nSize);
	offset += nSize;
}

void CPacket::GenerateHeader(bool hEncrypted)
{
	if(hEncrypted)
	{
		offset -= 4;
...
bool CPacket::MapPacketBelow()
{
	...
	FileHandler.Write(0x31,1);
	FileHandler.Write((char *)data,unencLen+2);
